...
void percolate(void)
{
  ...
  for (i=0; i<NB; i++) {
    bond=order[i];
    row=(bond-bond%(NN*L/2))/(NN*L/2);
    if ((bond%(NN*L/2))<L) {s1=row*L+bond%(NN*L/2);s2=nn[s1][0];}
    else {s1=row*L+(bond%(NN*L/2)-L)*2+row%2;s2=nn[s1][1];}
  ...
  for (i=0; i<N; i++) {         //square lattice, 8 neighbors
    if ((i/L)%2) { // odd column index
      if (i%2){    //odd row
        ...
        nn[i][1] = (i-1+N)%(N);
        nn[i][2] = (i-L+N)%(N);
        if (i%L==0) {nn[i][1] = (i+L-1)%(N);}        //correct boundary at start and end of each row
      }
    }
    else{
        if (i%2){    //odd row
