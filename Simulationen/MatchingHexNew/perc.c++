...
void main(void)
{
  int decorated=0,face,i;
  rngseed(0);
  permutation_faces();
  boundaries();
   
  for(decorated=0;decorated<N/2;decorated++)
    {
      pc=0;sigmasq=0;
      if ((!(decorated%(N/200)))&&(((double)(decorated)/(N/2))>0.25))
	{
	  for (i=0;i<RUNS;i++)
	    {
	      permutation();
	  ...
	  cout <<RUNS<<"  "<<L<<"  "<<pow(L,-0.75)<<"   "<<(double)(decorated)/(N/2)<<"   "<<pc/RUNS<<"   "<<sqrt((sigmasq/(RUNS-1)-pc*pc/RUNS/RUNS)/(RUNS))<<"\n";
	}
      face=2*order_faces[decorated]+(2*order_faces[decorated]/L)%2;
      decorate(face);
    }
}
...
void permutation_faces(void)
{
  ...
  for (i=0; i<N/2; i++) order_faces[i] = i;
  for (i=0; i<N/2; i++) {
    j = i + (N/2-i)*RNGLONG;
    temp = order_faces[i];
